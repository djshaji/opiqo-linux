...
guitar/audio effects processor with a GTK4 desktop UI and a JACK audio backend.
...
src/
  ...
  linux/
    ...
    ParameterPanel.h/.cpp     ← Dynamic GTK4 controls from LV2 port metadata
    SettingsDialog.h/.cpp     ← Audio routing + preset import/export dialog
    opiqo.css                 ← Application stylesheet
    opiqo.gresource.xml       ← GResource manifest
...
**GTK4 file dialogs** — `GtkFileDialog` (GTK 4.10+) is used everywhere instead
of `GtkFileChooserNative` to ensure Wayland portal compatibility and avoid
deprecated APIs.
...
```bash
cmake --preset linux-default        # Release build in build-linux/
