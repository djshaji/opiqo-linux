...
- provides a GTK4 desktop UI with 4 plugin slots in a 2×2 layout,
...
src/
  ...
  linux/
    ...
    ParameterPanel.h/.cpp     ← Dynamic GTK4 controls from PortInfo
...
if(OPIQO_TARGET_PLATFORM STREQUAL "linux")
  ...
  target_include_directories(opiqo PRIVATE
    ${GTK4_INCLUDE_DIRS} ${JACK_INCLUDE_DIRS} ${LILV_INCLUDE_DIRS}
    ${SNDFILE_INCLUDE_DIRS} src/ src/gtk4/
  )

  target_link_libraries(opiqo PRIVATE
    ${GTK4_LIBRARIES} ${JACK_LIBRARIES} ${LILV_LIBRARIES}
...
**Goals:** Compile a minimal GTK4 window under the new `linux-default` preset.
...
**Deliverables:** `build-linux/opiqo` binary that opens an empty GTK4 window.

**Acceptance criteria:**
- `cmake --preset linux-default && cmake --build build-linux` succeeds without
  warnings on a system with GTK4 ≥ 4.6 and JACK ≥ 1.9.
...
**Goals:** Build the full GTK4 shell layout.
...
GtkApplicationWindow "opiqo"
  GtkBox (vertical, spacing=0)
    ...
    GtkLabel "statusBar_"  ← bottom status text
```

- Define the static skeleton in a `main_window.ui` GtkBuilder file.
- `MainWindow::create()` calls
  `gtk_builder_new_from_resource("/opiqo/ui/main_window.ui")`.
- Minimum window size: 900 × 650 (set via `gtk_window_set_default_size`).
- CSS file `opiqo.css` loaded via `GtkCssProvider` for basic slot framing.
...
- Display with `GtkListView` + `GtkSingleSelection` + `GtkStringList`.
- Filter entry using `GtkSearchEntry` + `GtkFilterListModel`.
...
**Goals:** Generate GTK4 controls at runtime from `LV2Plugin::PortInfo`.

**Tasks:**

*Control type → GTK4 widget mapping:*

| `PortInfo::ControlType` | GTK4 widget |
|-------------------------|-------------|
| `Float` (range)         | `GtkScale` (horizontal, `GTK_ORIENTATION_HORIZONTAL`) |
| `Float` (enumeration)   | `GtkDropDown` with `GtkStringList` of scale-point labels |
| `Toggle`                | `GtkCheckButton` |
| `Trigger`               | `GtkButton` (fires `engine_->setValue(slot, idx, 1.0f)`) |
| `AtomFilePath`          | `GtkButton` "Browse…" → `GtkFileChooserNative` |
...
- Build loop calls `engine_->getPluginPortInfo(slot)` and appends a row
  (`GtkBox` horizontal: label + control) per port.
...
- For `AtomFilePath`, use `gtk_file_chooser_native_new()` to avoid GTK sandbox
  issues on Flatpak/Snap.
...
**Presets tab:**
- Export button: `GtkFileChooserNative` (save) → writes result of
  `engine_->getPresetList()` to JSON.
- Import button: `GtkFileChooserNative` (open) → reads JSON and calls
  `engine_->applyPreset()` per slot.
...
- Use `gtk_file_chooser_native_new()` for all file dialogs (required for
  Wayland portals).
- Avoid `GdkX11` and `GdkWayland` backend-specific APIs.
- Use `gdk_display_get_default()` only for font/DPI queries.
