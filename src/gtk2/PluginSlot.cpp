// src/gtk2/PluginSlot.cpp (GTK2 port)
...
void PluginSlot::buildWidgets() {
    ...
    // GTK2: gtk_vbox_new instead of gtk_box_new(GTK_ORIENTATION_VERTICAL, 0)
    GtkWidget* outerBox = gtk_vbox_new(FALSE, 0);
    gtk_container_add(GTK_CONTAINER(frame_), outerBox);

    // ── Header row ────────────────────────────────────────────────────────
    // GTK2: gtk_hbox_new instead of gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 4)
    headerBox_ = gtk_hbox_new(FALSE, 4);
    ...
    // GTK2: gtk_hseparator_new instead of gtk_separator_new(GTK_ORIENTATION_HORIZONTAL)
