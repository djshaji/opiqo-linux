// src/gtk2/MainWindow.cpp (GTK2 port)
...
// ── Embedded GTK RC style ─────────────────────────────────────────────────────

static const char* kAppRc = R"RC(
widget "*.delete-btn" style "delete-btn-style"
widget "*.confirm-btn" style "confirm-btn-style"
...
MainWindow::MainWindow() {
    ...
    // Wire JACK hot-plug change notification
...
void MainWindow::buildMenuBar(GtkWidget* root) {
    ...
    GtkWidget* quitItem = gtk_image_menu_item_new_from_stock(GTK_STOCK_QUIT, nullptr);
...
void MainWindow::buildWidgets() {
    ...
    // ── 2×2 plugin slot table ─────────────────────────────────────────────
    slotTable_ = gtk_table_new(2, 2, TRUE);   // 2 rows, 2 cols, homogeneous
    gtk_widget_set_size_request(slotTable_, -1, 400);
    gtk_box_pack_start(GTK_BOX(root), slotTable_, TRUE, TRUE, 0);

    // Create slots 1–4 (stored at indices 0–3)
    // layout: slot1→(0,1,0,1), slot2→(1,2,0,1), slot3→(0,1,1,2), slot4→(1,2,1,2)
    const int left[4]   = {0, 1, 0, 1};
    const int right[4]  = {1, 2, 1, 2};
    const int top[4]    = {0, 0, 1, 1};
    const int bottom[4] = {1, 1, 2, 2};
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        slot->setFileCallback  ([this](int s, const std::string& u,
            ...
            onSetFilePath(s, u, path);
        });

        gtk_table_attach(GTK_TABLE(slotTable_), slot->widget(),
...
void MainWindow::savePluginCache() const {
    ...
    const std::string path = std::string(dirname((char*)AppSettings::configPath().c_str()))
