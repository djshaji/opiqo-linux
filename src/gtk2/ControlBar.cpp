// src/gtk2/ControlBar.cpp (GTK2 port)
...
void ControlBar::buildWidgets() {
    // ── Outer bar ─────────────────────────────────────────────────────────
    // GTK2: gtk_hbox_new instead of gtk_box_new(GTK_ORIENTATION_HORIZONTAL, ...)
    bar_ = gtk_hbox_new(FALSE, 8);
    ...
    // GTK2: gtk_vseparator_new instead of gtk_separator_new(GTK_ORIENTATION_VERTICAL)
    ...
    // GTK2: gtk_combo_box_text_new (GTK 2.24+)
    ...
    // ── Quality box (hidden for WAV / FLAC) ───────────────────────────────
    ...
    // GTK2: must set no-show-all then hide to keep it hidden when show_all is called
    gtk_widget_set_no_show_all(qualityBox_, TRUE);
...
void ControlBar::onFormatChanged() {
    ...
    // GTK2: gtk_widget_show/hide instead of gtk_widget_set_visible
    if (lossy)
