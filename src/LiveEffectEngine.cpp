...
int LiveEffectEngine::addPlugin (int position, std::string uri) {
...

    {
        ...
        if ( !plugin->initialize()) {
            ...
            char * libname = basename(
                    (char *)lilv_node_as_string(lilv_plugin_get_library_uri(plugin->plugin_)));
...
std::string LiveEffectEngine::initPlugins () {
    ...
    LILV_FOREACH (plugins, i, plugins) {
        ...
        if (p == nullptr) {
            LOGE("[initPlugins] Failed to get plugin at index");
            continue;
        }
...
//        LOGD("[plugin] %s\n", lilv_node_as_uri(lilv_plugin_get_uri(p)));
        ...
        if (nameNode == nullptr || uriNode == nullptr) {
            LOGE("[initPlugins] Plugin at index has no name or URI, skipping");
        ...
        if (name == nullptr || uri == nullptr) {
            LOGE("[initPlugins] Failed to get name or uri for plugin at index ");
            lilv_node_free(nameNode);
            lilv_node_free(authorNode);
            continue;
        }
