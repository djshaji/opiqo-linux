...
void ControlBar::buildWidgets() {
    ...
    g_signal_connect_swapped(powerToggle_, "toggled",
                             ...
                             this);
    gtk_box_append(GTK_BOX(bar_), powerToggle_);
    ...
    gtk_box_append(GTK_BOX(bar_), gainLabel);
    ...
    g_signal_connect_swapped(gainScale_, "value-changed",
                             ...
                             this);
    gtk_box_append(GTK_BOX(bar_), gainScale_);
    ...
    gtk_box_append(GTK_BOX(bar_), fmtLabel);

    // Keep order in sync with FileType enum: WAV=0, MP3=1, OGG=2, OPUS=3, FLAC=4
    const char* formats[] = {"WAV", "MP3", "OGG", "OPUS", "FLAC", nullptr};
    GtkStringList* fmtList = gtk_string_list_new(formats);
    formatDrop_ = gtk_drop_down_new(G_LIST_MODEL(fmtList), nullptr);
    gtk_drop_down_set_selected(GTK_DROP_DOWN(formatDrop_), 0);
    gtk_widget_set_tooltip_text(formatDrop_, "Recording file format");
    g_signal_connect_swapped(formatDrop_, "notify::selected",
                             G_CALLBACK(+[](ControlBar* self, GParamSpec*) {
                                 self->onFormatChanged();
                             }),
                             this);
    gtk_box_append(GTK_BOX(bar_), formatDrop_);
    ...
    gtk_box_append(GTK_BOX(qualityBox_), qLabel);

    const char* qualities[] = {"0 (low)", "1", "2", "3", "4", "5 (med)",
                                "6", "7", "8", "9 (high)", nullptr};
    GtkStringList* qList = gtk_string_list_new(qualities);
    qualityDrop_ = gtk_drop_down_new(G_LIST_MODEL(qList), nullptr);
    gtk_drop_down_set_selected(GTK_DROP_DOWN(qualityDrop_), 5);
    gtk_box_append(GTK_BOX(qualityBox_), qualityDrop_);

    gtk_box_append(GTK_BOX(bar_), qualityBox_);
    gtk_widget_set_visible(qualityBox_, FALSE);  // WAV selected by default
    ...
    g_signal_connect_swapped(recordToggle_, "toggled",
                             ...
                             this);
    gtk_box_append(GTK_BOX(bar_), recordToggle_);

    // ── Spacer ────────────────────────────────────────────────────────────
    GtkWidget* spacer = gtk_box_new(GTK_ORIENTATION_HORIZONTAL, 0);
    gtk_widget_set_hexpand(spacer, TRUE);
    gtk_box_append(GTK_BOX(bar_), spacer);
    ...
    gtk_box_append(GTK_BOX(bar_), xrunLabel_);
    ...
    gtk_box_append(GTK_BOX(bar_), statusLabel_);
}
...
void ControlBar::onFormatChanged() {
    // Show quality selector only for lossy formats (MP3=1, OGG=2, OPUS=3)
    const int fmt = static_cast<int>(
        gtk_drop_down_get_selected(GTK_DROP_DOWN(formatDrop_)));
...
void ControlBar::onRecordToggled() {
    ...
    const int qual = static_cast<int>(
        gtk_drop_down_get_selected(GTK_DROP_DOWN(qualityDrop_)));
    if (recordCb_)
        recordCb_(active, fmt, qual);
}

// ── State setters (called from main thread) ────────────────────────────────────
