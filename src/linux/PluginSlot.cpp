...
void PluginSlot::buildWidgets() {
    ...
    gtk_widget_add_css_class(frame_, "plugin-slot");
    ...
    g_signal_connect_swapped(addButton_, "clicked",
        ...
        this);
    gtk_box_append(GTK_BOX(headerBox_), addButton_);
    ...
    g_signal_connect_swapped(deleteButton_, "clicked",
        ...
        this);
    gtk_box_append(GTK_BOX(headerBox_), deleteButton_);

    gtk_box_append(GTK_BOX(outerBox), headerBox_);
    gtk_box_append(GTK_BOX(outerBox), gtk_separator_new(GTK_ORIENTATION_HORIZONTAL));

    // ── Parameter panel (scrollable) ──────────────────────────────────────
    // Wire value callbacks from ParameterPanel up through the slot
    ...
    gtk_box_append(GTK_BOX(outerBox), paramPanel_->widget());
}
