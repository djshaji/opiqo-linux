...
guitar/audio effects processor with a GTK3 desktop UI and a JACK audio backend.
...
src/
  ...
  gtk3/
    ...
    ParameterPanel.h/.cpp     ← Dynamic GTK3 controls from LV2 port metadata
...
builds. Only `src/gtk3/` and `src/main_linux.cpp` are GTK3-specific.
...
**GTK3 file dialogs** — Uses `GtkFileChooserDialog` (synchronous or async) for
file selection. Some APIs differ from GTK4; see code comments for porting notes.
...
```bash
cmake --preset linux-gtk3        # Release build in build-linux-gtk3/
cmake --build build-linux-gtk3 -j$(nproc)
```

For a debug build:
```bash
cmake --preset linux-gtk3-debug
cmake --build build-linux-gtk3-debug -j$(nproc)
```
